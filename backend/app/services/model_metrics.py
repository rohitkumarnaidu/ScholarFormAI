...
class ModelMetrics:
    ...
    def _persist_metric(self, model: str, latency: float, success: bool, quality_score: Optional[float]):
        ...
        def _task():
            try:
                ...
                err = str(exc)
                missing_table = "model_metrics" in err and (
                    "schema cache" in err or "PGRST205" in err or "Could not find the table" in err
                )
                if missing_table:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_quality_scores": {
                model: sum(s["score"] for s in self.quality_scores if s["model"] == model)
                / max(1, len([s for s in self.quality_scores if s["model"] == model]))
    ...
    def get_model_comparison(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agent_vs_legacy": {
                ...
                "agent_success_rate": (
                    self.metrics["nvidia"]["successful_calls"] + self.metrics["deepseek"]["successful_calls"]
                )
                / (nvidia_calls + deepseek_calls),
                "legacy_success_rate": self.metrics["rules"]["successful_calls"] / rules_calls,
                "automation_level": "High"
                if self.metrics["rules"]["total_calls"] < (nvidia_calls + deepseek_calls) * 0.2
                else "Low - Reliance on Fallbacks",
            },
        }
