...
class TestContractLoader:
    ...
    def test_load_yaml_error(self):
        ...
        with (
            ...
            patch("builtins.open"),
            patch("yaml.safe_load", side_effect=Exception("yaml err")),
        ):
...
class TestEquationStandardizer:
    ...
    def test_process_exception_logged(self):
        ...
        doc.add_processing_stage.assert_called_once_with(
            stage_name="equation_standardization", status="error", message=ANY
        )
...
class TestQualityScorer:
    ...
    def test_section_balance_perfect(self):
        ...
        result = QualityScorer().score(
            {"sections": [{"title": "A", "content": "word " * 100}, {"title": "B", "content": "word " * 100}]},
            "ieee",
            {"sections": ["A", "B"]},
        )
...
class TestTaskParser:
    ...
    def test_parse(self):
        ...
        with patch(
            "app.pipeline.generation.task_parser.generate", return_value='{"doc_type": "essay", "title": "My Essay"}'
        ):
