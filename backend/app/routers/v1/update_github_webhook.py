...
async def github_release_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    channel = db.query(UpdateChannel).filter(UpdateChannel.app_id == app.id, UpdateChannel.name == channel_name).first()
    ...
    existing_release = (
        db.query(UpdateRelease).filter(UpdateRelease.app_id == app.id, UpdateRelease.version == tag_name).first()
    )
