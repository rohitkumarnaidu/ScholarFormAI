...
def trace_confidence_flow(docx_path: str):
    ...
    blocks_using_nlp_fallback = sum(
        1 for b in doc.blocks if b.metadata.get("classification_method") == "fallback_with_nlp"
    )
    blocks_using_last_resort = sum(
        1 for b in doc.blocks if b.metadata.get("classification_method") == "fallback_last_resort"
    )
    deterministic_blocks = sum(
        1
        for b in doc.blocks
        if "deterministic" in b.metadata.get("classification_method", "")
        or "structure" in b.metadata.get("classification_method", "")
    )
    ...
    print(
        f"block_type: {title_block.block_type.value if hasattr(title_block.block_type, 'value') else str(title_block.block_type)}"
    )
