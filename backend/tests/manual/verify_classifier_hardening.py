...
def test_classifier_hardening():
    ...
    blocks.append(
        Block(block_id="title", text="Forensic Audit of Pipeline Safety", index=0, block_type=BlockType.TITLE)
    )
    ...
    for i in range(1, 25):
        blocks.append(
            Block(
                block_id=f"b{i}",
                text="This is a very long paragraph of body text that goes on for a while to test the safety limit. "
                * 5,
                index=i,
    ...
    blocks.append(
        Block(block_id="h1", text="Header Text", index=25, block_type=BlockType.UNKNOWN, metadata={"is_header": True})
    )
    blocks.append(
        Block(block_id="f1", text="Footer 1", index=26, block_type=BlockType.UNKNOWN, metadata={"is_footer": True})
    )

    doc = PipelineDocument(document_id="test_hardening", filename="test.docx", blocks=blocks)
