...
def _has_413(send: AsyncMock) -> bool:
    return any(
        c[0][0].get("type") == "http.response.start" and c[0][0].get("status") == 413 for c in send.call_args_list
    )
...
class TestMaxBodySizeMiddleware:
    ...
    async def test_error_response_detail_message(self):
        ...
        await mw(scope, receive, send)
        body_calls = [c[0][0] for c in send.call_args_list if c[0][0].get("type") == "http.response.body"]
