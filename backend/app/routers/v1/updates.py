...
async def check_for_updates_post(request: UpdateCheckRequest, db: Session = Depends(get_db)):
    service = get_update_service(db)
    try:
        result = service.check_for_updates(
...
async def download_update(request: UpdateDownloadRequest, service: UpdateService = Depends(get_update_service)):
...
async def rollback_update(version: str | None = None, service: UpdateService = Depends(get_update_service)):
...
async def get_update_history(limit: int = 50, service: UpdateService = Depends(get_update_service)):
...
async def get_release_notes(version: str, service: UpdateService = Depends(get_update_service)):
...
async def update_update_settings(request: UpdateSettingsUpdate, service: UpdateService = Depends(get_update_service)):
