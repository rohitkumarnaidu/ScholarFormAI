  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="mx-auto max-w-6xl px-4 py-8">
        {/* Header */}
        <div className="mb-6">
          <Link href="/issues" className="mb-4 inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700">
            <ArrowLeft className="h-4 w-4" />
            Back to Issues
          </Link>
          <h1 className="text-2xl font-bold text-gray-900">Issue Admin</h1>
          <p className="text-sm text-gray-500 mt-1">Manage issue tracking, labels, milestones, and settings</p>
        </div>
          ...
          <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <div className="rounded-xl border border-gray-200 bg-white p-5">
                  <div className="flex items-center gap-3">
                    <div className="rounded-full bg-blue-100 p-2"><Bug className="h-4 w-4 text-blue-600" /></div>
                    <div>
                    ...
                    {Object.entries(stats.by_status || {}).map(([key, count]) => {
                      ...
                      return (
                        <div key={key} className="flex items-center gap-3">
                          <span className="w-24 text-xs capitalize text-gray-600">{key.replace("-", " ")}</span>
                          ...
                          <span className="w-10 text-right text-xs text-gray-500">{count} ({pct}%)</span>
                        </div>
                      );
                    })}
                  </div>
            ...
            <div className="rounded-xl border border-gray-200 bg-white p-5">
              ...
              {slaBreaches.length === 0 ? (
                <p className="text-sm text-gray-500">No SLA breaches detected.</p>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {slaBreaches.map((breach) => (
                        <tr key={breach.issue_id} className="border-b border-gray-100">
                          ...
                          <td className="py-2 text-gray-600">{breach.elapsed_hours.toFixed(1)}h</td>
                          <td className="py-2 text-gray-600">{breach.sla_hours}h</td>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            ...
            <div className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-gray-200 bg-gray-50 p-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 disabled:opacity-50 transition-colors"
              >
            ...
            <div className="space-y-2">
              {Object.keys(labels).length === 0 ? (
                ...
                Object.entries(labels).map(([key, label]) => (
                  <div key={key} className="flex items-center justify-between rounded-lg border border-gray-100 bg-white p-3">
                    <div className="flex items-center gap-3">
                      <span
              ...
              {milestones.length === 0 ? (
                ...
                milestones.map((ms, i) => (
                  <div key={i} className="flex items-center justify-between rounded-lg border border-gray-100 bg-white p-3">
                    <div>
                      <p className="text-sm font-medium text-gray-900">{String(ms.title || "")}</p>
                      {ms.description ? <p className="text-xs text-gray-500">{String(ms.description)}</p> : null}
                    </div>
                    <div className="flex items-center gap-3">
                      {ms.due_date ? (
                        <span className="text-xs text-gray-400">Due: {new Date(String(ms.due_date)).toLocaleDateString()}</span>
                      ) : null}
                      {ms.status ? (
        ...
        {activeTab === "settings" && (
          <div className="rounded-xl border border-gray-200 bg-white p-5">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Issue Settings</h2>

            <div className="space-y-4">
              {Object.entries(settings).length === 0 ? (
                <p className="text-sm text-gray-500">No configurable settings available.</p>
              ) : (
                  ...
                  <div key={key} className="flex items-center justify-between py-2">
                    ...
                    {typeof value === "boolean" ? (
                      ...
                      <input
                        ...
                        className="w-24 rounded-lg border border-gray-300 px-3 py-1.5 text-sm text-right"
                      />
                    ) : (
                      <input
                        type="text"
