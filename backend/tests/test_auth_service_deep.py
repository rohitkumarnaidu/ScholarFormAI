...
class TestLogin:
    ...
    def mock_supabase(self):
        with patch("app.services.auth_service.supabase") as mock_sb:
            response = type("Resp", (), {
                "model_dump": lambda self: {"access_token": "tok", "user": {"id": "u1"}}
            })()
    ...
    async def test_login_http_exception_passthrough(self, mock_supabase):
        mock_supabase.auth.sign_in_with_password.side_effect = HTTPException(
...
class TestForgotPassword:
    ...
    async def test_http_exception_passthrough(self, mock_supabase):
        mock_supabase.auth.reset_password_for_email.side_effect = HTTPException(
