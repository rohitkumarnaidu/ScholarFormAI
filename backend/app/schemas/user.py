...
class UserBase(BaseModel):
    ...
    institution: Optional[str] = Field(None, description="University or organisation the user belongs to.")
    role: str = Field(
        "authenticated",
...
class UserProfile(User):
    ...
    last_sign_in_at: Optional[datetime] = Field(None, description="Last successful login timestamp (UTC).")
    is_verified: bool = Field(False, description="Whether the user's email has been verified.")
    document_count: Optional[int] = Field(None, description="Total number of documents processed by this user.")
...
class UserUpdateRequest(BaseModel):
    ...
    institution: Optional[str] = Field(None, max_length=200, description="New institution name.")
