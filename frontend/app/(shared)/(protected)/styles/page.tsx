...
import { getStyles } from '@/services/api.v1';
...
export default function StylesPage() {
  ...
  const { data: fetchedStyles } = useQuery({
    queryKey: ['styles'],
    queryFn: () => getStyles(),
    staleTime: 5 * 60 * 1000,
  });

  const styles = Array.isArray(fetchedStyles) && fetchedStyles.length > 0 ? fetchedStyles : fallbackStyles;
