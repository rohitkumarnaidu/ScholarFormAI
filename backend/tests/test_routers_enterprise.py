...
class TestDocumentsImplEditDocument:
    ...
    async def test_missing_edited_data(self):
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await edit_document(mock_request, "d1", {"not_edited_data": {}}, MagicMock(), current_user=MagicMock(id="u1"))
    ...
    async def test_doc_not_found(self):
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await edit_document(mock_request, "nonexistent", {"edited_structured_data": {"k": "v"}}, MagicMock(), current_user=MagicMock(id="u1"))
    ...
    async def test_not_authorized(self):
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await edit_document(mock_request, "d1", {"edited_structured_data": {"k": "v"}}, MagicMock(), current_user=MagicMock(id="u1"))
...
class TestDocumentsImplDownload:
    ...
    async def test_not_authorized(self):
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await download_document(mock_request, "d1", format="docx", token=None, expires=None, current_user=MagicMock(id="u1"))
            assert exc.value.status_code == 403
    ...
    async def test_pdf_export_runtime_error(self):
        ...
        mock_request.url = MagicMock()
        mock_request.url.path = "/api/v1/documents/d1/download"
        doc = {"id": "d1", "user_id": None, "status": "COMPLETED", "output_path": "/tmp/doc.docx", "output_hash": None, "filename": "paper.docx"}
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await download_document(mock_request, "d1", format="pdf", token="valid", expires=9999999999, current_user=None)
            assert exc.value.status_code == 400
    ...
    async def test_tex_export_runtime_error(self):
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await download_document(mock_request, "d1", format="tex", token="valid", expires=9999999999, current_user=None)
            assert exc.value.status_code == 400
...
class TestDocumentsImplBatchUpload:
    ...
    async def test_too_many_files(self):
        ...
        with (
            ...
            with pytest.raises(HTTPException) as exc:
                await batch_upload(mock_request, MagicMock(), files=files, template="ieee", current_user=MagicMock(id="u1"))
            assert exc.value.status_code == 400
    ...
    async def test_invalid_extension(self):
        ...
        with (
            ...
            result = await batch_upload(mock_request, MagicMock(), files=[file1], template="ieee", current_user=MagicMock(id="u1"))
...
class TestDocumentsImplChunkedUpload:
    ...
    async def test_db_unavailable(self):
        ...
        mock_request = MagicMock()
        with patch("app.routers.v1.documents_impl._require_db", side_effect=HTTPException(status_code=503, detail="DB not configured")):
            with pytest.raises(HTTPException) as exc:
                await upload_document_chunked(mock_request, MagicMock(), file_id="safe", chunk_index=0, total_chunks=1, file=MagicMock(), current_user=MagicMock(id="u1"))
            assert exc.value.status_code == 503
...
class TestProviderHelpers:
    ...
    def test_record_provider_metrics_exception(self):
        from app.routers.v1.providers import _record_provider_metrics
        with patch("app.middleware.prometheus_metrics.MetricsManager.record_provider_operation", side_effect=Exception("fail")):
            _record_provider_metrics("test", "openai", "success")
...
class TestDocumentsRouterHelpers:
    ...
    def test_record_upload_ack_duration_exception(self):
        from app.routers.v1.documents_impl import _record_upload_ack_duration
        with patch("app.middleware.prometheus_metrics.MetricsManager.record_upload_ack_duration", side_effect=Exception("fail")):
            _record_upload_ack_duration(0.0)
...
class TestUnderscoreHelpers:
    ...
    def test_record_persona_kpis_exception(self):
        from app.routers.v1._helpers import _record_persona_kpis
        mock_request = MagicMock()
        with patch("app.middleware.prometheus_metrics.MetricsManager.record_persona_event", side_effect=Exception("fail")):
    ...
    def test_build_error_response_with_details(self):
        ...
        mock_request = MagicMock()
        mock_request.state.request_id = "req-1"
        response = build_error_response(mock_request, status_code=422, code="VALIDATION", message="invalid", details={"field": "name"})
        assert response.status_code == 422
...
class TestBillingHelpers:
    ...
    def test_lookup_user_id_by_customer_found(self):
        from app.routers.v1.billing import _lookup_user_id_by_customer
        sb = MagicMock()
        sb.table.return_value.select.return_value.eq.return_value.maybe_single.return_value.execute.return_value = MagicMock(data={"id": "user-1"})
        assert _lookup_user_id_by_customer(sb, "cus_1") == "user-1"

    def test_lookup_user_id_by_customer_exception(self):
        from app.routers.v1.billing import _lookup_user_id_by_customer
        sb = MagicMock()
        sb.table.return_value.select.return_value.eq.return_value.maybe_single.return_value.execute.side_effect = Exception("fail")
        assert _lookup_user_id_by_customer(sb, "cus_1") is None
