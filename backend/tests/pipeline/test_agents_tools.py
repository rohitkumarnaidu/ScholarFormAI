...
class TestToolRegistry:
    ...
    def test_register(self):
        ...
        tool_cls = r.register(
            name="my_tool", description="My custom tool", input_schema={"x": (str, "Input param")}, execute_fn=my_fn
        )
...
class TestGlobalCustomTools:
    ...
    def test_create_citation_formatter_tool_apa(self):
        ...
        result = instance._run(
            authors=["Smith, J.", "Doe, A.", "Lee, B.", "Kim, C."], title="Test Paper", year="2024", style="apa"
        )
...
class TestToolMarketplace:
    ...
    def test_publish_tool_no_tags(self, tmp_path):
        ...
        tm = ToolMarketplace(local_cache_dir=str(tmp_path))
        result = tm.publish_tool(
            tool_name="simple_tool", tool_code="print(1)", description="Simple", author="u", version="0.0.1"
        )
        assert result["success"] is True
