...
class TestComparisonDashboard:
    def test_generate_html(self, tmp_path):
        ...
        tracker.get_summary.return_value = {
            ...
            "total_runs": 8, "last_updated": "2026-01-01"
        }
        tracker.get_comparison.return_value = {
            "agent_vs_legacy": {
                "speed": {"agent_avg_duration": 15.2, "legacy_avg_duration": 30.5},
                "quality": {"agent_success_rate": 0.95, "legacy_success_rate": 0.80,
                            "agent_avg_errors": 0.5, "legacy_avg_errors": 2.1},
    ...
    def test_build_html_with_data(self):
        ...
        summary = {
            ...
            "total_runs": 18, "last_updated": "2026-06-01"
        }
        comp = {
            "agent_vs_legacy": {
                "speed": {"agent_avg_duration": 12.5, "legacy_avg_duration": 25.0},
                "quality": {"agent_success_rate": 0.9, "legacy_success_rate": 0.7,
                            "agent_avg_errors": 1.0, "legacy_avg_errors": 3.0},
...
class TestAdvancedAnalyticsDashboard:
    ...
    def test_generate_html_with_deps(self, tmp_path):
        ...
        mdl.get_insights_summary.return_value = {
            "total_authors": 5, "total_venues": 3, "document_types": 2,
            "quality_trend_count": 10, "top_authors": [("Author A", {"document_count": 3, "avg_references": 15})], "top_venues": []
        }
        ads = MagicMock()
        ads.get_config.return_value = {"max_retries": 3, "timeout_seconds": 60, "fallback_threshold": 0.5, "enable_caching": True}
        dc = MagicMock()
    ...
    def test_build_adaptive_section_with_data(self):
        from app.pipeline.agents.advanced_dashboard import AdvancedAnalyticsDashboard
        ads = MagicMock()
        ads.get_config.return_value = {"max_retries": 5, "timeout_seconds": 120, "fallback_threshold": 0.3, "enable_caching": False}
    ...
    def test_build_distributed_section_with_data(self):
        ...
        dc.get_statistics.return_value = {
            "total_tasks": 200, "specialists": {"formatter": {"task_count": 80}, "parser": {"task_count": 120}}
        }
...
class TestNextGenDashboard:
    ...
    def test_generate_html_with_deps(self, tmp_path):
        ...
        td.get_summary.return_value = {
            "model_name": "scibert", "device": "cpu", "cached_embeddings": 50,
            "n_clusters": 3, "clusters_trained": True
        }
        fn = MagicMock()
        fn.get_status.return_value = {
            "node_id": "node-1", "local_updates": 10, "global_model_version": 2, "coordinator_connected": True
        }
        rt = MagicMock()
        rt.get_current_params.return_value = {"timeout": 30.0, "retry_enabled": True, "aggressive_mode": False}
        am = MagicMock()
        am.get_statistics.return_value = {
            "current_workers": 5, "min_workers": 2, "max_workers": 10,
            "avg_cpu_percent": 45.3, "total_scaling_events": 3
        }
        tm = MagicMock()
    ...
    def test_build_transformer_section_with_data(self):
        ...
        td = MagicMock()
        td.get_summary.return_value = {
            "model_name": "bert-base", "device": "cuda", "cached_embeddings": 100,
            "n_clusters": 5, "clusters_trained": True
        }
    ...
    def test_build_transformer_section_not_trained(self):
        ...
        td = MagicMock()
        td.get_summary.return_value = {
            "model_name": "test", "device": "cpu", "cached_embeddings": 0,
            "n_clusters": 0, "clusters_trained": False
        }
        d = NextGenDashboard(transformer_detector=td)
        html = d._build_transformer_section()
        assert "Not Trained" in html
    ...
    def test_build_federated_section_with_data(self):
        ...
        fn = MagicMock()
        fn.get_status.return_value = {
            "node_id": "abc-123", "local_updates": 25, "global_model_version": 3, "coordinator_connected": True
        }
    ...
    def test_build_federated_section_offline(self):
        ...
        fn = MagicMock()
        fn.get_status.return_value = {
            "node_id": "x", "local_updates": 0, "global_model_version": 1, "coordinator_connected": False
        }
        d = NextGenDashboard(federated_node=fn)
        html = d._build_federated_section()
    ...
    def test_build_autoscaling_section_with_data(self):
        ...
        am = MagicMock()
        am.get_statistics.return_value = {
            "current_workers": 3, "min_workers": 1, "max_workers": 8,
            "avg_cpu_percent": 62.5, "total_scaling_events": 7
        }
