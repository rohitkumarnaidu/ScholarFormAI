...
class Reference(BaseModel):
    ...
    # Unique identifier
    reference_id: str = Field(..., description="Unique identifier (e.g., 'ref_001')")

    # Sequential numbering (assigned by formatting stage)
    number: Optional[int] = Field(default=None, description="Sequential reference number (e.g., 1 for '[1]')")

    # Citation key (e.g., 'Smith2020', '[1]', 'Smi20')
    citation_key: str = Field(..., description="Key used for in-text citations")

    # Raw reference text (as appears in document)
    raw_text: str = Field(..., description="Original reference text from document")

    # Reference type
    reference_type: ReferenceType = Field(default=ReferenceType.UNKNOWN, description="Type of referenced work")

    # Parsed bibliographic fields
    # --- Authors ---
    authors: List[str] = Field(
        default_factory=list, description="List of author names (e.g., ['Smith, J.', 'Doe, A.'])"
    )
    ...
    # --- Publication venue ---
    journal: Optional[str] = Field(default=None, description="Journal name (for articles)")
    conference: Optional[str] = Field(default=None, description="Conference name (for conference papers)")
    book_title: Optional[str] = Field(default=None, description="Book title (for books or book chapters)")
    ...
    issn: Optional[str] = Field(default=None, description="ISSN (for journals)")
    url: Optional[str] = Field(default=None, description="Web URL")
    arxiv_id: Optional[str] = Field(default=None, description="arXiv identifier (for preprints)")

    # --- Other fields ---
    edition: Optional[str] = Field(default=None, description="Edition (for books)")
    note: Optional[str] = Field(default=None, description="Additional notes")

    # Position in document
    block_id: Optional[str] = Field(default=None, description="Block ID of this reference in the references section")
    index: int = Field(..., description="Sequential position in reference list (0-based)")

    # Citation tracking
    cited_by: List[str] = Field(default_factory=list, description="List of block IDs that cite this reference")
    ...
    style: Optional[CitationStyle] = Field(default=None, description="Citation style to use for formatting")

    # Validation
    is_valid: bool = Field(default=True, description="Whether reference passed validation")
    warnings: List[str] = Field(default_factory=list, description="Validation warnings (e.g., missing required fields)")
    is_complete: bool = Field(
        default=False, description="Whether all required fields for the reference type are present"
    )
