...
import {
  useIssueStats, useSLABreaches, useLabels, useMilestones, useIssueSettings,
  useCreateLabel, useDeleteLabel, useCreateMilestone, useUpdateIssueSettings
...
export default function AdminPage() {
  ...
  const { data: slaBreachesData, isLoading: slaLoading } = useSLABreaches();
  ...
  const { data: settingsData, isLoading: settingsLoading } = useIssueSettings();
  ...
  const slaBreaches = slaBreachesData || [];
  const labels = labelsData || {};
  const milestones = milestonesData || [];
  
  const loading = statsLoading || slaLoading || labelsLoading || milestonesLoading || settingsLoading;
  ...
  const updateSettingsMutation = useUpdateIssueSettings();

  const handleCreateLabel = () => {
    if (!newLabelName.trim()) return;
    createLabelMutation.mutate({ name: newLabelName.trim(), color: newLabelColor, description: newLabelDesc || undefined }, {
      onSuccess: () => {
        setNewLabelName("");
        setNewLabelDesc("");
      }
  ...
  const handleDeleteLabel = (key: string) => {
    deleteLabelMutation.mutate(key);
  };

  const handleCreateMilestone = () => {
    if (!newMsTitle.trim()) return;
    createMilestoneMutation.mutate({ title: newMsTitle.trim(), description: newMsDesc || undefined, dueDate: newMsDue || undefined }, {
      onSuccess: () => {
        setNewMsTitle("");
        setNewMsDesc("");
        setNewMsDue("");
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white p-5">
            ...
            <div className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 p-4">
              ...
              <button
                ...
                disabled={createMilestoneMutation.isPending || !newMsTitle.trim()}
                className="inline-flex items-center gap-1 rounded-lg bg-accent-500 px-4 py-2 text-sm font-medium text-white hover:bg-accent-400 disabled:opacity-50 transition-colors"
              >
