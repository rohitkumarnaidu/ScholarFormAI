...
class DocumentAgent:
    ...
    def __init__(
        ...
        try:
            ...
            if self.llm is None:
                self.llm = CustomLLMFactory.create_llm(provider=llm_provider, model=llm_model, temperature=temperature)
        ...
        try:
            ...
            system_prompt = (
                "You are a document processing agent. Use the available tools to analyze and process documents."
            )
...
"""
            ...
            if self.memory:
                self.memory.remember_pattern("document_processing", context, success=True)
    ...
    def _run_direct_fallback(self, document: Optional[PipelineDocument], doc_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = {"output": "\n\n".join(analysis_parts), "intermediate_steps": intermediate_steps}
            return {
                "success": True,
                "analysis": result.get("output", ""),
                "intermediate_steps": result.get("intermediate_steps", []),
    ...
    def _execute_with_retry(self, input_message: str) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                if self.executor is None:
                    raise RuntimeError(f"Agent executor unavailable due to import error: {self._agent_import_error}")
    ...
    def _should_fallback(self, result: Dict[str, Any]) -> bool:
        ...
        if len(intermediate_steps) > 0 and error_count / len(intermediate_steps) > 0.5:
            logger.warning(
                f"High error rate detected ({error_count}/{len(intermediate_steps)}). Recommending fallback."
            )
            return True
