...
class TestScanUploadedFile:
    ...
    async def test_malware_detected(self):
        with (
            patch("app.routers.v1.documents_impl.virus_scanner") as mock_scanner,
...
class TestGetStatus:
    ...
    async def test_status_from_db(self):
        ...
        with (
            ...
            patch("app.routers.v1.documents_impl._set_cached_status_response"),
            patch(
                "app.routers.v1.documents_impl._extract_quality_payload",
                return_value={"quality_score": 90, "quality_summary": None, "quality": None},
            ),
        ):
