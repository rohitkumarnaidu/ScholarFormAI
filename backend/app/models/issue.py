...
class Issue(Base):
    ...
    type = Column(String, nullable=False) # e.g., 'bug', 'feature_request', 'feedback', 'performance', 'security', 'crash'
    ...
    user = relationship("User", foreign_keys=[user_id], lazy="selectin", primaryjoin="Issue.user_id == foreign(User.id)")
    assignee = relationship("User", foreign_keys=[assignee_id], lazy="selectin", primaryjoin="Issue.assignee_id == foreign(User.id)")
