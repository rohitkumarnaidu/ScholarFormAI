...
class TestBuildErrorResponse:
    ...
    def test_with_details(self):
        ...
        request = MagicMock()
        request.state.request_id = "req-1"
        resp = build_error_response(request, status_code=400, code="BAD", message="bad", details={"field": "error"})
...
class TestRecordPersonaKPIs:
    ...
    def test_silent_fail_on_exception(self):
        with patch(
            "app.middleware.prometheus_metrics.MetricsManager.record_persona_event", side_effect=Exception("boom")
        ):
            from app.routers.v1._helpers import _record_persona_kpis
...
class TestRunEnveloped:
    ...
    def test_success_with_custom_status_code(self, request_mock):
        ...
        result = run_enveloped(request_mock, AsyncMock(return_value={"ok": True}), success_status_code=201)
        import asyncio
