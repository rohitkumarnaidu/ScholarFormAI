...
import { ChevronDown, ChevronLeft, ChevronRight, CloudDownload, Loader2, PlusCircle, Search, SearchX, X, Ruler, GraduationCap, Monitor, FlaskConical, FileText, Dna, BookOpen, Library, BookMarked, Pill, ListOrdered, Filter, Droplet, Sparkles, Palette } from 'lucide-react';

import { useBuiltinTemplates, useSearchCSLStyles, useFetchCSLStyle } from '@/src/hooks/useTemplates';

// Built-in templates with stable UI metadata.
const BUILT_IN_TEMPLATES = [
    { id: 'ieee', name: 'IEEE', description: 'IEEE manuscript style for engineering and technical papers.', category: 'Engineering', citation: 'Numbered [1]', icon: Ruler, available: true },
    { id: 'apa', name: 'APA (7th)', description: 'APA 7th Edition formatting for social and behavioral sciences.', category: 'Social Science', citation: 'Author-Year', icon: GraduationCap, available: true },
    { id: 'acm', name: 'ACM', description: 'ACM format for computer science conference and journal submissions.', category: 'Computer Science', citation: 'ACM Numbered', icon: Monitor, available: true },
    { id: 'springer', name: 'Springer', description: 'Springer-ready structure and references for scientific manuscripts.', category: 'Science', citation: 'Author-Year', icon: FlaskConical, available: true },
    { id: 'elsevier', name: 'Elsevier', description: 'Elsevier manuscript formatting with numbered reference conventions.', category: 'Science', citation: 'Numbered', icon: FileText, available: true },
    { id: 'nature', name: 'Nature', description: 'Nature-style manuscript setup with superscript citation expectations.', category: 'Biology/Science', citation: 'Superscript', icon: Dna, available: true },
    { id: 'harvard', name: 'Harvard', description: 'Harvard referencing and document conventions for general use.', category: 'General', citation: 'Author-Year', icon: BookOpen, available: true },
    { id: 'chicago', name: 'Chicago (17th)', description: 'Chicago Notes-Bibliography formatting for humanities writing.', category: 'Humanities', citation: 'Notes-Bib', icon: Library, available: true },
    { id: 'mla', name: 'MLA (9th)', description: 'MLA 9th Edition formatting for language and literature manuscripts.', category: 'Humanities', citation: 'Author-Page', icon: BookMarked, available: true },
    { id: 'vancouver', name: 'Vancouver', description: 'Vancouver numeric style commonly used in medical publishing.', category: 'Medicine', citation: 'Numbered', icon: Pill, available: true },
    { id: 'numeric', name: 'Numeric', description: 'Generic numbered citation template for broad journal compatibility.', category: 'General', citation: 'Numbered', icon: ListOrdered, available: true },
    { id: 'none', name: 'None', description: 'Preserves document structure with minimal formatting intervention.', category: 'Passthrough', citation: 'Preserved', icon: Filter, available: true },
    { id: 'modern_blue', name: 'Modern Blue', description: 'Modern visual treatment with blue accents and IEEE-like citation behavior.', category: 'Custom', citation: 'IEEE-based', icon: Droplet, available: true },
    { id: 'modern_gold', name: 'Modern Gold', description: 'Modern premium style with gold accents and APA-like citation behavior.', category: 'Custom', citation: 'APA-based', icon: Sparkles, available: true },
...
export default function Templates() {
    ...
    const [cslResults, setCslResults] = useState<any[]>([]);
    const [cslImporting, setCslImporting] = useState<string | null>(null);
    const [cslBanner, setCslBanner] = useState<{ type: 'success' | 'error' | 'info'; msg: string } | null>(null);

    const { data: apiTemplates, isLoading } = useBuiltinTemplates();
    const searchCslMutation = useSearchCSLStyles();
    const importCslMutation = useFetchCSLStyle();

    const templates = useMemo(() => {
        const templatesData = (apiTemplates as any)?.templates ?? apiTemplates ?? [];
        if (Array.isArray(templatesData) && templatesData.length > 0) {
            const mappedTemplates = templatesData.map((template: any) => {
                ...
                const metadata: any = BUILT_IN_TEMPLATE_MAP.get(templateId) || {};
                return {
                    ...metadata,
                    id: templateId,
                    ...
                    available: template.available !== false && (metadata as any).available !== false,
            ...
            return [...allKnownTemplates, ...additionalTemplates];
        }
    ...
    const handlePreviewGuidelines = (template: any) => {
        setPreviewTemplate(template);
    };

    const handleCslSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (!cslQuery.trim()) return;
        setCslResults([]);
        setCslBanner(null);
        searchCslMutation.mutate(cslQuery.trim(), {
            onSuccess: (cslData) => {
    ...
    const handleCslImport = (slug: string, name: string) => {
        setCslImporting(slug);
        setCslBanner(null);
        importCslMutation.mutate(slug, {
            onSuccess: () => {
                        ...
                        <form onSubmit={handleCslSearch} className="flex gap-3 max-w-2xl mb-2">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    disabled={searchCslMutation.isPending || Boolean(cslImporting)}
                                />
                            </div>
                            <button
                                type="submit"
                                disabled={!cslQuery.trim() || searchCslMutation.isPending || Boolean(cslImporting)}
