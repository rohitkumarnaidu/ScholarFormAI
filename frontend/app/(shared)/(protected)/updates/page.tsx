...
import {
  useUpdateSettings, useUpdateChannels, useUpdateHistory,
  useSaveUpdateSettings, useCheckForUpdates, useDownloadUpdate,
  useInstallUpdate, useRollbackUpdate
...
export default function SettingsPage() {
  ...
  const { data: settingsData, isLoading: settingsLoading } = useUpdateSettings();
  const { data: channelsData, isLoading: channelsLoading } = useUpdateChannels();
  const { data: historyData, isLoading: historyLoading } = useUpdateHistory(10);
  ...
  const rollbackMutation = useRollbackUpdate();
  const saveSettingsMutation = useSaveUpdateSettings();
      ...
      onSuccess: (result) => {
        if (result.success) {
          ...
          installMutation.mutate(undefined, {
            ...
            onError: (e) => setMessage({ type: "error", text: `Install Error: ${e instanceof Error ? e.message : "Unknown"}` })
          });
        } else {
          setMessage({ type: "error", text: result.error || "Download failed" });
        }
      },
      onError: (e) => setMessage({ type: "error", text: `Download Error: ${e instanceof Error ? e.message : "Unknown"}` })
    });
  };
  ...
  const handleRollback = () => {
    ...
    rollbackMutation.mutate(undefined, {
      ...
      onError: (e) => setMessage({ type: "error", text: `Rollback Error: ${e instanceof Error ? e.message : "Unknown"}` })
    });
  };
                    ...
                    <div className="flex-1">
                      ...
                      {checkResult.update?.changelog && (
                        <ul className="mt-2 text-sm text-slate-600 dark:text-slate-400 list-disc list-inside space-y-0.5">
                          {checkResult.update.changelog.slice(0, 5).map((c: string, i: number) => <li key={i}>{c}</li>)}
                        </ul>
                      )}
                      {checkResult.status === "update-available" && (
                        <div className="mt-3 flex gap-2">
                          <button
                            ...
                            disabled={downloadMutation.isPending || installMutation.isPending}
                          ...
                          >
                            ...
                            {downloadMutation.isPending || installMutation.isPending ? "Downloading..." : "Download & Install"}
                          </button>
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white p-6">
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg bg-accent-500 px-4 py-2 text-sm font-medium text-white hover:bg-accent-400 disabled:opacity-50 transition-colors"
              >
                <Save className={`h-4 w-4 ${saveSettingsMutation.isPending ? "animate-spin" : ""}`} />
