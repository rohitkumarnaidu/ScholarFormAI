...
def main():
    ...
    results["week1_agent_a"].append(check_file("app/pipeline/services/grobid_client.py", "GROBID Client"))
    results["week1_agent_a"].append(check_file("tests/test_grobid_client.py", "GROBID Tests"))

    print("\n--- Agent B: CSL Citation Engine ---")
    results["week1_agent_b"].append(check_file("app/pipeline/services/csl_engine.py", "CSL Engine"))
    results["week1_agent_b"].append(check_file("app/templates/ieee/styles.csl", "IEEE CSL Style"))
    results["week1_agent_b"].append(check_file("app/templates/apa/styles.csl", "APA CSL Style"))
    results["week1_agent_b"].append(check_file("tests/test_csl_engine.py", "CSL Tests"))
    ...
    results["week2_agent_a"].append(check_file("app/pipeline/services/docling_client.py", "Docling Client"))
    results["week2_agent_a"].append(check_file("tests/test_docling_client.py", "Docling Tests"))
    results["week2_agent_a"].append(
        check_file("app/pipeline/structure_detection/detector.py", "Structure Detector (Enhanced)")
    )
    results["week2_agent_a"].append(check_file("tests/test_structure_detector_docling.py", "Structure Detector Tests"))

    print("\n--- Agent B: docxtpl Template Rendering ---")
    results["week2_agent_b"].append(check_file("app/pipeline/formatting/template_renderer.py", "Template Renderer"))
    results["week2_agent_b"].append(check_file("tests/test_template_renderer.py", "Template Renderer Tests"))
    ...
    print(
        f"Week 1 - Agent B (CSL):              {week1_b_pct:.0f}% ({sum(results['week1_agent_b'])}/{len(results['week1_agent_b'])})"
    )
    print(
        f"Week 2 - Agent A (Docling):          {week2_a_pct:.0f}% ({sum(results['week2_agent_a'])}/{len(results['week2_agent_a'])})"
    )
    print(
        f"Week 2 - Agent B (docxtpl):          {week2_b_pct:.0f}% ({sum(results['week2_agent_b'])}/{len(results['week2_agent_b'])})"
    )
    print(
        f"Week 3 - Agent A (Hard Guards):      {week3_a_pct:.0f}% ({sum(results['week3_agent_a'])}/{len(results['week3_agent_a'])})"
    )
    print(
        f"Week 3 - Agent B (Templates):        {week3_b_pct:.0f}% ({sum(results['week3_agent_b'])}/{len(results['week3_agent_b'])})"
    )

    overall_pct = calculate_completion([item for sublist in results.values() for item in sublist])
