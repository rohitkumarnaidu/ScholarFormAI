...
def client():
    ...
    with (
        ...
        patch("app.routers.v1.documents_impl.enhancement_manager.dispatch_document_pipeline", return_value={"mode": "immediate"}),
        patch("app.routers.v1.documents_impl.enhancement_manager.dispatch_edit_flow", return_value={"mode": "immediate"}),
...
class TestUploadChunked:
    def test_invalid_file_id(self, client):
        response = client.post(
            ...
            data={
                "file_id": "../../etc/passwd", "chunk_index": 0,
                "total_chunks": 1, "template": "ieee",
            },
    ...
    def test_chunk_exceeds_limit(self, client):
        response = client.post(
            "/api/v1/documents/upload/chunked",
            data={
                "file_id": "safe-id-123", "chunk_index": 0,
                "total_chunks": 1, "template": "ieee",
            },
    ...
    def test_single_chunk_received(self, client, tmp_path):
        ...
        response = client.post(
            "/api/v1/documents/upload/chunked",
            data={
                "file_id": "test-chunk-id", "chunk_index": 0,
                "total_chunks": 1, "template": "ieee",
            },
...
class TestListDocuments:
    def test_success(self, client):
        client.mock_ds.list_documents = AsyncMock(return_value=[
            {
                "id": "d1", "filename": "test.docx", "template": "ieee",
                "status": "COMPLETED", "progress": 100, "current_stage": "EXPORT",
...
class TestGetStatus:
    def test_success(self, client):
        client.mock_ds.get_document = AsyncMock(return_value={
            "id": "job-1", "status": "PROCESSING", "current_stage": "PARSING",
            "progress": 45, "error_message": None, "user_id": "user-123",
            "created_at": "now", "updated_at": "now",
        })
        client.mock_ds.get_processing_statuses = AsyncMock(return_value=[
            {"phase": "UPLOAD", "status": "success", "message": "done",
    ...
    def test_not_authorized(self, client):
        client.mock_ds.get_document = AsyncMock(return_value={
            "id": "job-1", "status": "PROCESSING", "user_id": "other-user",
        })
        client.mock_ds.get_processing_statuses = AsyncMock(return_value=[])
...
class TestGetSummary:
    def test_success(self, client):
        client.mock_ds.get_document = AsyncMock(return_value={
            "id": "d1", "user_id": "user-123", "status": "COMPLETED",
            "filename": "paper.docx", "template": "ieee",
            "created_at": "now", "output_path": "/tmp/doc.docx",
        })
        client.mock_ds.get_document_result = AsyncMock(return_value={
...
class TestEditDocument:
    def test_success(self, client):
        client.mock_ds.get_document = AsyncMock(return_value={
            "id": "d1", "user_id": "user-123", "template": "ieee",
            "filename": "test.docx",
        })
...
class TestGetPreview:
    def test_success(self, client):
        client.mock_ds.get_document = AsyncMock(return_value={
            "id": "d1", "user_id": "user-123", "filename": "p.pdf",
            "template": "apa", "status": "COMPLETED", "created_at": "now",
        })
        client.mock_ds.get_document_result = AsyncMock(return_value={
...
class TestGetComparisonData:
    ...
    def test_not_ready(self, client):
        client.mock_ds.get_document = AsyncMock(return_value={
            "id": "d1", "user_id": "user-123", "status": "PROCESSING",
        })
...
class TestDelete:
    def test_success(self, client):
        client.mock_ds.get_document = AsyncMock(return_value={
            "id": "d1", "user_id": "user-123", "filename": "test.docx",
            "output_path": None, "original_file_path": None,
        })
