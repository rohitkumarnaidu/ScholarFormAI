...
class TestDocumentGenerator:
    ...
    def test_default_session_config(self, dg):
        cfg = dg._default_session_config(
            doc_type="paper", template="ieee", metadata={"k": "v"}, options={"opt": True}, user_id="u1"
        )
    ...
    def test_session_record_to_status(self, dg):
        record = {
            "id": "j1",
            "config_json": {"stage": "writing", "progress": 50, "message": "working", "status": "processing"},
        }
    ...
    def test_session_record_to_status_error(self, dg):
        record = {
            "id": "j1",
            "config_json": {"error": "boom", "status": "failed", "stage": "error", "progress": 0, "message": "fail"},
        }
        result = dg._session_record_to_status(record)
...
class TestPromptBuilder:
    def test_build_academic_paper(self):
        ...
        result = PromptBuilder().build(
            "academic_paper", meta, {"include_placeholder_content": True, "word_count_target": 2000}
        )
...
class TestReferenceFormatterEngine:
    ...
    def test_format_all(self):
        ...
        with patch.object(rfe.csl_engine, "format_references", side_effect=Exception("CSL unavailable")):
            refs = [
                MagicMock(authors=["Smith, J."], title="Paper", year="2020", raw_text="raw", reference_type="article")
            ]
...
class TestFormatterMethods:
    ...
    def test_apply_initial_layout(self):
        ...
        with patch.object(
            f.contract_loader,
            "load",
            return_value={"layout": {"margins": {"top": 1, "bottom": 1, "left": 1, "right": 1}}},
        ):
