...
class TestNormalizer:
    ...
    def test_normalize_blocks_abstract_split(self):
        ...
        b.metadata = {}
        b.model_copy = lambda **kw: MagicMock(
            text=kw.get("update", {}).get("text", b.text),
            index=b.index,
            block_id=kw.get("update", {}).get("block_id", b.block_id),
            style=b.style,
            metadata=b.metadata,
        )
    ...
    def test_normalize_blocks_consecutive_duplicate(self):
        ...
        b1.metadata = {}
        b1.warnings = []
        b1.model_copy = lambda **kw: MagicMock(
            text=kw.get("update", {}).get("text", "Hello"), index=b1.index, style=shared_style, metadata={}, warnings=[]
        )
        b2 = MagicMock()
        ...
        b2.metadata = {}
        b2.warnings = []
        b2.model_copy = lambda **kw: MagicMock(
            text=kw.get("update", {}).get("text", "Hello"), index=b2.index, style=shared_style, metadata={}, warnings=[]
        )
        with patch("app.pipeline.normalization.normalizer.normalize_block_text", side_effect=lambda x, **kw: x):
