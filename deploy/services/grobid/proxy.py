...
def health() -> Dict[str, str]:
    try:
        resp = requests.get(f"{TARGET_BASE}/api/isalive", timeout=5)
        if resp.status_code == 200:
            return {"status": "ok", "service": "grobid-proxy", "grobid": "reachable"}
        return {"status": "error", "service": "grobid-proxy", "grobid": f"responded {resp.status_code}"}
    except requests.RequestException:
        return {"status": "error", "service": "grobid-proxy", "grobid": "unreachable"}
