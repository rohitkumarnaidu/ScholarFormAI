...
class PerformanceTracker:
    ...
    def _calculate_stats(self, metrics_list: List[Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_references": sum(m.get("references_count", 0) for m in successful) / len(successful)
            if successful
            else 0,
            "avg_figures": sum(m.get("figures_count", 0) for m in successful) / len(successful) if successful else 0,
            "avg_validation_errors": sum(m.get("validation_errors", 0) for m in successful) / len(successful)
            if successful
            else 0,
            "fallback_rate": sum(1 for m in metrics_list if m.get("fallback_triggered", False)) / len(metrics_list)
            if metrics_list
            else 0,
        }
        ...
        return {
            "agent_vs_legacy": {
                "speed": {
                    ...
                    "speedup": legacy.get("avg_duration", 1) / agent.get("avg_duration", 1)
                    if agent.get("avg_duration")
                    else 0,
                },
                "quality": {
