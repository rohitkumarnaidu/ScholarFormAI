...
def detect_numbering_pattern(text: str) -> dict[str, Any] | None:
    ...
    if decimal_match:
        ...
        return {"pattern_type": "decimal", "number": number, "level": level, "remainder": remainder}
    ...
    if roman_match:
        ...
        return {"pattern_type": "roman", "number": number, "level": 1, "remainder": remainder}
...
def detect_title(block: Block, all_blocks: list) -> bool:
    ...
    non_empty = [
        b for b in all_blocks if b.text.strip() and not b.metadata.get("is_header") and not b.metadata.get("is_footer")
    ]
    if not non_empty:
        return False
...
def analyze_heading_candidate(
    ...
    if not num_info and not matches_section_keyword(text):
        ...
        if block_index < 5 or (
            style_score > 0.4 and avg_font_size and block.style.font_size and block.style.font_size > avg_font_size
        ):
            level = 1
