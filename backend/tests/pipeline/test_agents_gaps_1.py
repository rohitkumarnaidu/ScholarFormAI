...
class TestMultiDocumentLearnerInit:
    ...
    def test_loads_existing_insights(self, tmp_path):
        ...
        d = tmp_path / "mdl"
        d.mkdir()
        (d / "insights.json").write_text(
            json.dumps({"author_patterns": {"Dr A": {"document_count": 1, "avg_references": 10.0, "avg_quality": 1.0}}})
        )
...
class TestMultiDocumentLearnerRecord:
    ...
    def test_multiple_documents_average(self, tmp_path):
        ...
        l.record_document(
            "d1",
            {"authors": ["Bob"], "venue": "ICML"},
            {"references_count": 10, "success": True, "figures_count": 2, "duration_seconds": 100},
        )
        l.record_document(
            "d2",
            {"authors": ["Bob"], "venue": "ICML"},
            {"references_count": 20, "success": False, "figures_count": 4, "duration_seconds": 200},
        )
