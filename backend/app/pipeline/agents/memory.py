...
class AgentMemory:
    ...
    def record_metric(self, metric_name: str, value: float, metadata: dict | None = None):
        ...
        if metric_name not in self.metrics:
            self.metrics[metric_name] = {"values": [], "average": 0.0, "count": 0}
    ...
    def get_memory_summary(self) -> dict[str, Any]:
        ...
        return {
            "patterns": {
                pattern_type: {"successful_count": len(data["successful"]), "failed_count": len(data["failed"])}
                for pattern_type, data in self.patterns.items()
            },
            "errors": {"total_errors": len(self.errors), "unique_types": len(set(e["type"] for e in self.errors))},
            "metrics": {
                metric_name: {"average": data["average"], "count": data["count"]}
    ...
    def format_memory_summary(self) -> str:
        ...
        if pat:
            for ptype, data in pat.items():
                lines.append(f"  - {ptype}: {data['successful_count']} successful, {data['failed_count']} failed")
        else:
            lines.append("  (none)")
        err = summary.get("errors", {})
        lines.append(f"Errors: {err.get('total_errors', 0)} total, {err.get('unique_types', 0)} unique types")
